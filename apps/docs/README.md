...
Daytona provides a documentation website ([daytona.io/docs](https://www.daytona.io/docs)) covering core features, SDKs, API, and CLI references, integration guides, and examples.

### Structure

Documentation resides in the [`apps/docs`](../../apps/docs) directory. It's built with Astro and Starlight, and authored in Markdown (`.md`) and Markdown Extended (`.mdx`) files. The documentation website is deployed at [daytona.io/docs](https://www.daytona.io/docs) in server-rendered (SSR) mode using an Express adapter, with custom middleware handling redirects and routing.
...
└── src/
    ...
    │   ├── docs/
    │   │   ├── en/                     # Documentation content (localized)
    │   │   │   ├── <filename>.mdx      # Documentation page
    │   │   │   ├── guides/             # Integration guides
    │   │   │   ├── tools/              # CLI and API reference
...
yarn nx build docs --configuration=production
```
...
Documentation includes automation scripts in the `tools/` directory for maintaining auto-generated content.
...
```bash
node apps/docs/tools/update-cli-reference.js
```
...
```bash
node apps/docs/tools/update-api-reference.js
```
...
```bash
node apps/docs/tools/update-llms.js
```

- **Input**: `.mdx` and `.md` files from `src/content/docs/en/`
...
```bash
node apps/docs/tools/update-search.js
```
