...
- `cursor?` _string_ - Pagination cursor from a previous response
...
- `limit?` _number_ - Number of sandboxes per page
...
- `order?` _"asc" \| "desc"_ - Sort direction
- `snapshots?` _string\[\]_ - Filter by snapshot names
- `sort?` _"name" \| "cpu" \| "memoryGiB" \| "diskGiB" \| "lastActivityAt" \| "createdAt"_ - Sort by field
- `states?` _string\[\]_ - Filter by states
...
- `items` _Sandbox\[\]_ - List of Sandboxes in the current page
- `nextCursor` _string_ - Cursor for the next page of results
    
    
    ListSandboxesResponse
    - _Inherited from_: `ListSandboxesResponse.nextCursor`



- `Omit`\<`ListSandboxesResponseDto`, `"items"`\>