...
ECR access uses cross-account IAM role assumption: you create a role in
your AWS account that trusts Daytona's runner principal, and Daytona
assumes it on every pull to fetch a short-lived ECR token. **No long-lived
AWS credentials are shared with Daytona, and no manual token rotation is
required.**

##### 1. Get the values from Daytona

Open [Daytona Registries ↗](https://app.daytona.io/dashboard/registries)
and copy:

- **Daytona Broker ARN** — the IAM principal to grant assume-role access to.
- **External ID** — your Daytona organization ID; used to scope trust to
  your organization.

##### 2. Create an IAM role in your AWS account

In your AWS account, create an IAM role with the trust and permissions
policies below. Replace `<DAYTONA_BROKER_ARN>` with the broker ARN and
`<YOUR_EXTERNAL_ID>` with your Daytona organization ID from the previous
step.
...
```json
{
  ...
  "Statement": [{
    "Effect": "Allow",
    "Principal": { "AWS": "<DAYTONA_BROKER_ARN>" },
...
Copy the ARN of the role you just created (e.g.
`arn:aws:iam::123456789012:role/daytona-ecr-puller`).

##### 3. Register the registry in Daytona

1. On [Daytona Registries ↗](https://app.daytona.io/dashboard/registries),
   click the **Add Registry** button.
2. Enter the **registry name**, **registry URL**, and **username**:

- **Registry URL**: `<account_id>.dkr.ecr.<region>.amazonaws.com`
- **Username**: ARN of the IAM role created in step 2
- **Password**: leave blank — Daytona fills it with your External ID
  automatically
...
1. Navigate to [Daytona Snapshots ↗](https://app.daytona.io/dashboard/snapshots).
2. Click the **Create Snapshot** button.
3. Enter the **snapshot name** and **image** using the full image
   reference, including the registry location and repository name
   (e.g. **`123456789012.dkr.ecr.us-east-1.amazonaws.com/my-repo/custom-alpine:3.21`**).

##### Optional: harden the trust policy

Daytona's runner sends a `daytona-<orgId>-pull` session name on every
AssumeRole call, which you can require in your trust policy for CloudTrail
auditability. Add inside `Condition`:
