...
Daytona provides two sets of runner management endpoints: [custom runner](#custom-regions) (`/runners`) and [admin](#admin-operations) (`/admin/runners`). While both support creating, listing, updating, and deleting runners, they serve different purposes and offer different levels of control.

Custom runner endpoints are scoped to your organization and require the `X-Daytona-Organization-ID` header. Admin endpoints operate across the entire platform and do not require an organization header.

|               | **Custom runner endpoints**                                                  | **Admin endpoints**                                                                                                                                                                                            |
| ------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Scope         | Organization                                                                 | Platform-wide                                                                                                                                                                                                  |
| Configuration | Minimal (**`name`**, **`regionId`**); API key is auto-generated and returned | Full configuration; requires **`apiKey`** and **`apiVersion`**, and specifying resource capacity (**`cpu`**, **`memoryGiB`**, **`diskGiB`**) and network settings (**`domain`**, **`apiUrl`**, **`proxyUrl`**) |
| Response      | Returns **`Runner`**                                                         | Returns **`RunnerFull`** (includes additional **`apiKey`** and **`regionType`** fields)                                                                                                                        |
| Filtering     | No filtering support                                                         | Supports filtering by **`regionId`** query parameter                                                                                                                                                           |

Both `Runner` and `RunnerFull` share the same base fields, including resource metrics such as CPU/memory/disk usage percentages, allocated resources, availability score, snapshot count, and started sandboxes. The `RunnerFull` type extends `Runner` with the runner's `apiKey` and `regionType`.
...
To request access, contact [support@daytona.io](mailto:support@daytona.io).
:::
...
### Update runner scheduling (admin)
...
:::note
Detailed installation instructions for the runner application will be provided in a future update. For assistance with runner installation, contact [support@daytona.io](mailto:support@daytona.io).
:::
